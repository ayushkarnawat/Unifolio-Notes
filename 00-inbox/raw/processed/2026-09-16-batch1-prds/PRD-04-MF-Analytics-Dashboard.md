...
**Nifty Midcap 150** (the "250" and "150" in the original brief refer to these — confirmed). NSE Indices'
...
**Industry-standard fund rating methodology, and what's replicable from public data
(added after initial draft):** Morningstar's star rating — the most widely recognized
fund-rating methodology globally — ranks funds within their category using a
risk-adjusted return measure computed from monthly returns, deliberately weighting
downside variation more heavily than upside variation (an application of expected-utility
theory, not a plain symmetric-risk measure like a standard Sharpe ratio), then buckets
funds into percentile tiers within category (top 10% highest tier, next 22.5%, middle
35%, next 22.5%, bottom 10% lowest), requires a minimum return history (Morningstar
uses three years) and a minimum category size (Morningstar requires at least 5 funds in
a category) before rating anything. This entire approach is computable from NAV history
alone — no portfolio holdings needed — which is why it's the basis for FR-5/FR-5a below.
Unifolio computes its own version of this measure rather than reproducing Morningstar's
exact proprietary formula (which isn't published in full and is a commercial product), and
layers on an explicit cost (TER) adjustment that general-purpose star ratings don't
directly incorporate — this is where the scorer becomes a genuine product decision
rather than a copy of an existing agency's number.

**Scheme-wise AUM is publicly available, which resolves the weighting question.**
AMFI publishes scheme-wise Average AUM (AAUM) disclosures — SEBI has mandated
monthly AAUM disclosure by AMCs since 2014, and AMFI's own aggregate scheme-wise
AAUM report (`amfiindia.com/aum-data/average-aum` and the related AUM-disclosure
filterable report) is at minimum quarterly at the aggregate level. This is enough
granularity for category-average weighting purposes — unlike NAV, AUM doesn't need
daily freshness for this use case, so a quarterly refresh cadence is an acceptable
...
- FR-4: Category-average comparison — show the held scheme's return against the
  **AUM-weighted** average of its category. AMFI publishes scheme-wise Average AUM
  (AAUM) disclosures (currently quarterly at the aggregate report level, per SEBI's
  monthly-disclosure mandate to individual AMC sites) — sufficient for this purpose,
  since AUM weighting doesn't need daily freshness the way NAV does. Resolved:
  AUM-weighted, not simple average — see Research.

#### Fund & Portfolio Scorer (industry-grade methodology, adapted to available data)
- FR-5: Score each fund via a **downside-weighted, risk-adjusted return measure**
  computed from monthly NAV returns over the available history (minimum 3 years of
  history required to score a fund at all — a fund younger than that shows "insufficient
  history" per Edge Cases, not a score). This is the same category of methodology
  Morningstar and CRISIL use industry-wide (risk-adjusted return that penalizes
  downside volatility more heavily than upside volatility, rather than a plain Sharpe
  ratio that penalizes both symmetrically) — Unifolio computes its own version of this
  from public NAV data rather than replicating any single rating agency's proprietary
  formula.
- FR-5a: Within each SEBI category, rank all schemes (not just the user's holdings — the
  full category universe, per the Research findings above) by that risk-adjusted-return
  measure, and bucket into the same percentile structure the industry standard uses:
  top 10% of category = highest tier, next 22.5%, middle 35%, next 22.5%, bottom 10% =
  lowest tier. A category needs at least 5 schemes with sufficient history to be rated at
  all — thin categories show "insufficient category data" per Edge Cases.
- FR-5b: **Cost overlay** — after the risk-adjusted-return tier is set, apply a secondary
  cost-efficiency adjustment based on the scheme's TER relative to its category average
  (from FR-10's TER data): meaningfully below-average TER nudges the score up within
  its tier, meaningfully above-average nudges it down. This is the piece that makes the
  score genuinely useful for a cost-conscious investor rather than a pure performance
  copy of an existing agency's rating — exact nudge magnitude is an implementation
  detail, not a product decision needing further sign-off, since the direction (reward low
  cost) is unambiguous.
...
- FR-7: The scorer must be explained, not just displayed — a short "why this score"
  breakdown per fund (risk-adjusted return tier + cost adjustment shown separately),
  so it doesn't read as an unexplained black-box number (this matters especially given
  the "100% accuracy" bar — a scorer is inherently a modeling choice, not a fact, and
...
| AMFI scheme-wise AAUM disclosure automation approach | Claude (technical) | Data source confirmed public, quarterly cadence at aggregate level | FR-4's AUM-weighting blocked until resolved; same low-risk category as TER/NSE automation |
| Portfolio-holdings data source (cap-wise composition, overlap) | Unresolved, shared with PRD-03 | Deliberately deferred per your decision — see Standing Reminder in Open Questions | US-7 stays deferred, revisit later as a combined spike with PRD-03 |
| Scorer methodology | Ayush + Claude | **Decided** — industry-grade risk-adjusted-return-within-category approach (FR-5–FR-5b), inspired by Morningstar's public methodology, computed independently from NAV data plus a Unifolio-specific cost overlay | None |
...
- [ ] Category ranking/comparison return window(s) — Morningstar's model uses 3/5/10yr
      with a weighted overall; propose the same (3yr minimum to qualify, 5yr blended in
      once available, 10yr likely too long given most Indian equity schemes' track
      records) — needs your confirmation, not a hard technical constraint — Owner: Ayush
- [ ] Exact cost-overlay nudge magnitude in FR-5b (how much a below/above-average TER
      shifts the score within its tier) — implementation detail, doesn't need product
      sign-off before build starts — Owner: Claude (technical), FYI to Ayush

---

**📌 Standing reminder — not forgotten, deliberately deferred:** cap-wise composition
(true large/mid/small-cap breakdown within a fund) and stock-level fund overlap both
remain out of scope for this PRD, per your decision to wait rather than open a dedicated
research spike now. Revisit this once the core Analytics Dashboard ships — the
underlying data gap (SEBI-mandated monthly AMC portfolio disclosures, not centrally
aggregated) is shared with PRD-03's equity look-through gap, so when this does get
picked up, it should be one combined research/build effort covering both, not two
separate ones.
...
- AMFI scheme-wise AAUM disclosure: amfiindia.com/aum-data/average-aum and
  amfiindia.com/aum-data/aum-disclosure (official, confirmed structure)
...
- TER regulatory basis: SEBI (Mutual Funds) Regulations, 1996, Regulation 52; general TER
  ...
  center, 2025–2026)
- AAUM regulatory basis: SEBI's 2014 monthly-AAUM-disclosure circular (Business
  Standard coverage, 2014, still current practice per AMFI's own AAUM page)
- Fund rating methodology reference: Morningstar Rating for Funds methodology
  documentation (Morningstar, 2021–2026 sources) — used as a public-methodology
  reference for FR-5/FR-5a's approach, not reproduced verbatim
...
| 1.1 | 2026-07-22 | Claude (PM partner) | Confirmed Nifty index mapping; resolved category-average weighting to AUM-weighted (AMFI publishes scheme-wise AAUM); replaced open scorer-weighting question with an industry-grade methodology (FR-5–FR-5b, Morningstar-style risk-adjusted-return-within-category ranking plus a Unifolio cost overlay); cap-wise composition/overlap research spike deliberately deferred per decision, standing reminder added |
