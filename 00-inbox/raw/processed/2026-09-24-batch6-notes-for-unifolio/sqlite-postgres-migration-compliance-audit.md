...
### F1 — CRITICAL: ImportStatus enum: 14 values used, only 3 exist at the database level
...
### F2 — CRITICAL: `opening_balance` transaction type missing from the SQLite CHECK constraint
...
### F3 — HIGH: No uniqueness enforcement on `household_members`' "self" row, at any level
...
### F5 — HIGH: `Database-Schema-Unifolio.md` is stale by three to four migrations
...
### F6 — HIGH: Runtime dialect branch in live NAV-caching logic, untested on Postgres
...
### F7 — MEDIUM: `compute_holdings` per-folio N+1 query pattern — confirmed still present
...
### F9 — LOW: One un-migrated async-blocking straggler remains: `amfi_aaum_client.py`
...
### F10 — LOW: Model/migration type declaration mismatch on `folios.coverage_gap_details`
