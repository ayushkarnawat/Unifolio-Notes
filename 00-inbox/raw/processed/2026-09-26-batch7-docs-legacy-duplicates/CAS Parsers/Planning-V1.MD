Path	User data?	Cost/effort	Verdict
CAS PDF parsing (open-source casparser Python lib)	✅ Full transaction history	Free, ~a day	Build this now — user downloads their own CAS from CAMS/MFCentral, uploads it. Fully legitimate (their data, their consent), works today, and it's the same fallback the whole industry moved to
casparser.in hosted API	✅	₹999/mo, free 10 credits	Fallback/benchmark against your own parser
mfapi.in	❌ (NAV/scheme data only)	Free, no key	Use for enrichment — daily NAVs, historical NAV series for valuation charts
parse.bot MFCentral API	❌ (public scheme data)	Key needed	Optional enrichment (sector/holdings data)
smallcase Gateway	✅	Commercial partnership, KYB	Not viable for a test build
Account Aggregator (via TSP like Setu/Finvu)	✅ real-time	Regulated entity, 5–10 months, ₹5–25L	Your long-term differentiator, not the prototype
