...
2. **RESOLVED.** The database schema didn't exist yet. Fixed via an SSM
   ...
   from this vault copy per the hard rule against reproducing secrets; see source at
   00-inbox/raw/2026-09-22-batch3-engineering-loop/session.md if the original value is
   ever needed for reference]`) contains a literal `$R` and `$5` —
