...
## Signature Element: "Fund Signal"

The original product brief asked to "display mutual funds in a very innovative, unique
way, if possible, not just a static name." Research (below) turned this from a vague wish
into a concrete, defensible direction rather than an unresolved placeholder.

### What the research found

- **Every one of the 11 direct/indirect competitors reviewed, and Mprofit itself,
  displays holdings as a static row: name, numbers, done** (confirmed against the
  Combined Feature-Parity Matrix and PRD-03's own competitive baseline). Nothing in
  this specific market gives an individual fund its own at-a-glance visual identity beyond
  a logo icon.
- **Outside this specific market, two patterns are well-established and proven, just not
  combined with each other or applied here:** (1) stock-trading apps (the Robinhood
  school of design) commonly pair each holding with a small sparkline showing its recent
  price trend, right in the list row — instantly recognizable, not decorative. (2) Current
  fintech design-system practice explicitly recommends that an asset's detail density
  *adapt*: full analytics on a larger view, collapsing to "a simplified trend line and a
  single status indicator" in a compact view — i.e., the sparkline-plus-status pattern is
  considered current best practice for asset-dense interfaces, not a novelty.
- **Unifolio's own logomark already introduces a shape language nothing else in this
  market uses**: the arc inside the "o" reads as a partial gauge/dial. No competitor
  reviewed uses radial/arc visual language anywhere in their product.

### The concept

**Fund Signal**: every holding gets a small radial arc (reusing the exact geometry
established in the logomark, not a generic progress-ring) whose fill represents that
fund's performance over a selectable period, paired with an optional sparkline on
expansion or a wider viewport. Direction (gain/loss) uses the semantic color system
(Color Discipline), not the brand accent, keeping brand identity and performance
signal cleanly separated per that section's existing rule.

This is a synthesis, not a copy of any single source: the *arc* comes from Unifolio's own
already-locked brand mark (nothing else in the product currently extends the logo's
geometry into the UI — this fixes that gap), the *sparkline-on-expand* pattern comes from
established fintech design-system practice generally (not any one named competitor),
and the *combination* of the two, applied to individual MF holdings specifically, is not
something any of the 11 reviewed competitors or Mprofit currently do. It also satisfies
Principle 2 (game-like pacing, never game-like mechanics) — an arc filling in as data
loads is a reveal, not a badge or a point.

This is a direction, not a finished design — actual visual exploration (does the arc read
clearly at holdings-table size, how it behaves in a dense list of 30+ funds, dark-mode
legibility) is real prototyping work for the Design Schema stage, not resolved by
describing it in prose. Flagged as the first concrete prototyping task there.
...
- [ ] Fund Signal (arc + sparkline) needs actual visual prototyping — does it read clearly
      at table row size, how it behaves across 30+ holdings, dark-mode legibility — the
      *direction* is settled (see Signature Element above), the *execution* is Design
      Schema work — Owner: Claude, with Ayush review

**Resolved:** no additional mood boards, sketches, or design skeleton are coming beyond
the brand identity already shared — confirmed decision, not a gap. Claude has full
creative latitude for direction, grounded in competitor/industry research rather than
copied from any single source (see Signature Element for how that standard was
applied).
...
- Combined Feature-Parity Matrix (project knowledge) — confirmed no competitor gives
  individual funds a distinct visual identity beyond a static row

### Research Sources
- Investment dashboard UX patterns (donut allocation charts, grouped benchmark bars,
  color+icon accessibility for gain/loss): Lollypop Design, 2026 investment-dashboard UX
  guide
- Asset-card responsive density pattern (full analytics on desktop, sparkline + status on
  compact views): Gapsy Studio fintech design-system guide, 2026
- Brand-color-as-trust-signal precedent (consistent accent color reinforcing recognition):
  Duck.Design fintech app design practices, 2025–2026
- Mutual fund and investment app UI/UX case studies reviewed for baseline patterns:
  Medium (Kshitiz Khandal, Groww/Dhan/Upstox comparison), Rillusion Wealth Fund case
  study, Dribbble/Behance mutual-fund dashboard collections (2025–2026)
...
| 1.1 | 2026-07-22 | Claude (PM partner) | Signature Element resolved from placeholder to a concrete, research-backed concept ("Fund Signal" — logo-derived arc + sparkline, synthesized from industry patterns not copied from any single competitor); confirmed no further design references are coming, Claude has creative latitude within researched constraints |
